...
export default function Profile({ user }: { user: UserData }) {
  return (
    <>
      <Button
        variant="ghost"
        className="md:hidden cursor-pointer relative h-10 w-10 rounded-full p-0 hover:bg-accent"
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            className="hidden md:block cursor-pointer relative h-10 w-10 rounded-full p-0 hover:bg-accent"
            aria-label="Profile menu"
          >
            {user?.image ? (
              <img
